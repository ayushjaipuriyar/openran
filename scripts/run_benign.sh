#!/bin/bash
#
# Benign Experiment Runner
# Orchestrates RIC, 5G Core, gNB, UEs, and traffic generation for benign experiments
# Usage: sudo ./run_benign.sh [start_training_set] [start_experiment_number] [only_exp]
#
# Note: This script requires sudo privileges for network namespace management,
#       Docker operations, and system-level network configuration.
#
...
# Simple color output functions (defined early for usage check)
info() { echo -ne "\n🔵 \033[1;34m$*\033[0m"; }
error() { echo -e "\n❌ \033[1;31m$*\033[0m"; }
warn() { echo -e "\n⚠️  \033[1;33m$*\033[0m"; }

if ! [[ "$START_TR" =~ ^[0-9]+$ && "$START_EXP" =~ ^[0-9]+$ ]]; then
	error "Usage: sudo $0 [start_training_set] [start_experiment_number] [only_exp]"
	error "Example: sudo $0 0 1"
	exit 1
fi
...
# Get the actual user (not root) for proper file ownership
ACTUAL_USER="${SUDO_USER:-$USER}"
ACTUAL_UID="${SUDO_UID:-$(id -u)}"
ACTUAL_GID="${SUDO_GID:-$(id -g)}"

info "Running as: root (actual user: $ACTUAL_USER)"
...
TOTAL_EXPERIMENTS=1
BASE_EXP_DIR="$PROJECT_ROOT/generated_experiments"
BASE_IP="10.45.1.2"
...
# State file will be set per experiment in the experiment directory
...
save_experiment_state() {
	...
	# Set state file path inside the experiment directory
	local exp_dir="${BASE_EXP_DIR}/tr${tr}/exp${exp}"
	local state_file="${exp_dir}/experiment_state.json"

	# Create experiment directory if it doesn't exist
	mkdir -p "$exp_dir" 2>/dev/null || return 0

	# Ensure we can write to the state file
	if [[ -f "$state_file" ]] && [[ ! -w "$state_file" ]]; then
		rm -f "$state_file" 2>/dev/null || {
			# Silently continue - state file is optional
			return 0
		}
	fi

	# Try to write state file, but don't fail if we can't
	if ! cat >"$state_file" 2>/dev/null <<EOF; then
{
    "training_set": $tr,
    "experiment": $exp,
    "step": "$step",
...
EOF
	...
	# Set proper ownership (best effort)
	chown "$ACTUAL_UID:$ACTUAL_GID" "$state_file" 2>/dev/null || true
...
cleanup() {
	...
	(
		cd "$PROJECT_ROOT/srsRAN_Project/docker"
		timeout 30s sudo docker compose down -v --remove-orphans >/dev/null 2>&1 || {
...
start_gnb() {
	...
	info "  TX Port: $tx_port, RX Port: $rx_port"
	...
	info "Checking metrics port $metrics_port availability..."
	if ! check_port_availability "$metrics_port"; then
		error "Cannot start gNB - metrics port $metrics_port unavailable"
		return 1
	fi
	success "Metrics port $metrics_port is available"

	# Start metrics server with better error handling
	info "Starting metrics server on port $metrics_port"
	local metrics_log="${log_dir}/${name}_metrics_server.log"

	# Ensure we can write to the log file
	touch "$metrics_log" 2>/dev/null || {
		error "Cannot create metrics server log file: $metrics_log"
		error "Check directory permissions: $(ls -ld "$log_dir")"
		return 1
	}

	python3 "$PROJECT_ROOT/lib/metrics_server.py" \
		--ip "$METRICS_IP" \
		-p "$metrics_port" \
		-f "${log_dir}/${name}_metrics.jsonl" >"$metrics_log" 2>&1 &

	local metrics_server_pid=$!
	echo $metrics_server_pid >/tmp/metrics_server.pid

	info "Metrics server started with PID: $metrics_server_pid"

	# Wait for metrics server to be ready with better diagnostics
	local wait_count=0
	local max_wait=30
	info "Waiting for metrics server to bind to port $metrics_port (max ${max_wait}s)..."

	while ((wait_count < max_wait)); do
		# Check if process is still running
		if ! kill -0 $metrics_server_pid 2>/dev/null; then
			error "Metrics server process died unexpectedly"
			if [[ -f "$metrics_log" ]]; then
				error "Metrics server log:"
				cat "$metrics_log" | sed 's/^/    /'
			fi
			return 1
		fi

		# Check if port is bound using multiple methods
		if lsof -i :"$metrics_port" 2>/dev/null | grep -q python; then
			success "Metrics server ready on port $metrics_port (took ${wait_count}s)"
			break
		fi
		...
		# Check using ss as another fallback
		if ss -tuln 2>/dev/null | grep -q ":${metrics_port} "; then
			success "Metrics server ready on port $metrics_port (took ${wait_count}s)"
			break
		fi
	...
	if ((wait_count >= max_wait)); then
		error "Metrics server failed to start within ${max_wait} seconds"
		error "Metrics server log:"
		[[ -f "$metrics_log" ]] && cat "$metrics_log" | sed 's/^/    /'
		error "Process status:"
		ps aux | grep -E "metrics_server|$metrics_server_pid" | grep -v grep | sed 's/^/    /'
		error "Port status:"
		lsof -i :"$metrics_port" 2>&1 | sed 's/^/    /'
		netstat -tuln | grep "$metrics_port" | sed 's/^/    /'
		kill $metrics_server_pid 2>/dev/null || true
		return 1
	fi

	# Check bind IP availability
	info "Checking bind IP $bind_ip availability..."
	if ss -tuln | grep -q "$bind_ip"; then
		...
		success "Bind IP $bind_ip is available"
	fi
	...
	info "Starting gNB in tmux session 'gnb_${name}'..."
	info "  Config: $GNB_CONF"
	info "  ZMQ TX: tcp://127.0.0.1:${tx_port}"
	info "  ZMQ RX: tcp://127.0.0.1:${rx_port}"

	# Build the command with proper quoting - use eval-safe format
	tmux new-session -d -s "gnb_${name}" bash -c "
		sudo gnb -c \"$GNB_CONF\" \\
		--ran_node_name \"$name\" \\
		--gnb_id 411 \\
		--gnb_cu_up_id 0 \\
	...
	sleep 2
	if ! tmux has-session -t "gnb_${name}" 2>/dev/null; then
		error "gNB tmux session died immediately after creation"
		error "Check gNB logs at: ${log_dir}/${name}_stdout.log"
		[[ -f "${log_dir}/${name}_stdout.log" ]] && tail -n 20 "${log_dir}/${name}_stdout.log" | sed 's/^/    /'
		return 1
	fi
	info "gNB tmux session is running"

	# Give gNB time to initialize
	info "Waiting 5s for gNB to initialize..."
	sleep 5

	# Wait for gNB connections with retry
	info "Waiting for gNB to establish N2 and E2 connections..."
	if ! wait_for_gnb_connection "$name" "$log_dir" 180; then
		error "gNB $name failed to establish connections"
		error "Last 30 lines of gNB log:"
		[[ -f "${log_dir}/${name}.log" ]] && tail -n 30 "${log_dir}/${name}.log" | sed 's/^/    /'
		return 1
	fi
...
start_ue() {
	...
	# Start UE in tmux session with proper quoting
	tmux new-session -d -s "ue$i" bash -c "
		sudo srsue \"${UE_CONF}\" \\
		--gw.netns \"$ue_ns\" \\
...
run_single_experiment() {
	...
	info "Conditions file: $conditions_file"
	info "Scenario script: $run_scenario_sh"
	info "Metrics CSV: $metrics_csv"

	save_experiment_state "$tr_num" "$exp_num" "starting" "$retry_count"

	# Validate required files exist
	info "Validating experiment files..."
	if [[ ! -f "$conditions_file" ]]; then
		error "Conditions file not found: $conditions_file"
		return 1
	fi
	success "Conditions file exists"

	if [[ ! -f "$run_scenario_sh" ]]; then
		error "Scenario script not found: $run_scenario_sh"
		return 1
	fi
	success "Scenario script exists"
	...
	info "Creating experiment directories..."
	mkdir -p "${exp_dir}/gnb_logs" "${exp_dir}/ue_logs" "${exp_dir}/metrics" || {
		error "Failed to create experiment directories"
		return 1
	}

	# Set proper ownership and permissions
	chown -R "$ACTUAL_UID:$ACTUAL_GID" "${exp_dir}" 2>/dev/null || true
	chmod -R 755 "${exp_dir}/gnb_logs" "${exp_dir}/ue_logs" "${exp_dir}/metrics" 2>/dev/null || true

	info "Directory permissions:"
	ls -ld "${exp_dir}" "${exp_dir}/gnb_logs" "${exp_dir}/ue_logs" "${exp_dir}/metrics" | sed 's/^/    /'

	success "Experiment directories created and ready (owner: $ACTUAL_USER)"

	# Step 1: Start Near-RT RIC
	step "[1/6] Starting Near-RT RIC"
	info "Changing to RIC directory: $PROJECT_ROOT/oran-sc-ric"
	save_experiment_state "$tr_num" "$exp_num" "ric_starting" "$retry_count"

	cd "$PROJECT_ROOT/oran-sc-ric"
	info "Starting RIC Docker Compose stack..."
	info "METRICS_PATH=${exp_dir}/metrics"
	...
	if ! retry_with_backoff 3 10 "RIC startup" \
		...
		sudo docker compose logs --tail=50 | sed 's/^/    /'
		return 1
	fi
	success "RIC Docker Compose stack started"

	info "Checking RIC service health..."
	if ! check_docker_service_health "ric_submgr"; then
		error "Near-RT RIC failed to become ready"
		error "RIC submgr logs:"
		sudo docker logs ric_submgr --tail=50 | sed 's/^/    /'
		return 1
	fi
	success "Near-RT RIC is healthy and ready"

	# Give RIC additional time to fully initialize E2 termination
	info "Waiting additional 10s for RIC E2 termination to be fully ready..."
	sleep 10

	# Verify E2 termination is ready
	info "Checking E2 termination status..."
	if sudo docker logs ric_e2term 2>&1 | grep -q "Listening"; then
		success "E2 termination is listening for connections"
	else
		warn "E2 termination may not be fully ready yet"
		info "E2term logs:"
		sudo docker logs ric_e2term --tail=20 | sed 's/^/    /'
	fi

	# Step 2: Start Open5GS Core
	step "[2/6] Starting Open5GS Core"
	info "Changing to 5GC directory: $PROJECT_ROOT/srsRAN_Project/docker"
	save_experiment_state "$tr_num" "$exp_num" "5gc_starting" "$retry_count"

	cd "$PROJECT_ROOT/srsRAN_Project/docker"
	info "Starting Open5GS Docker Compose stack..."
	...
	success "Open5GS Docker Compose stack started"

	info "Checking Open5GS health (timeout: 180s)..."
	if ! check_docker_service_health "open5gs_5gc" 180; then
		error "Open5GS Core failed to become healthy"
		error "Open5GS logs:"
		sudo docker logs open5gs_5gc --tail=50 | sed 's/^/    /'
		return 1
	fi
	success "Open5GS Core is healthy"
	...
	for port in 5201 5202 5203; do
		...
		if docker exec -d open5gs_5gc iperf3 -s -p "$port"; then
			success "iPerf server started on port $port"
		else
			warn "Failed to start iPerf server on port $port"
		fi
	done
	info "Waiting 3s for iPerf servers to stabilize..."
	sleep 3
	...
	cd "$PROJECT_ROOT"
	info "Checking and freeing ZMQ ports..."
	if ! check_and_free_zmq_ports; then
		error "Failed to prepare ZMQ ports"
		return 1
	fi
	success "ZMQ ports are ready"

	info "Starting gNB cu_cp_01..."
	if ! retry_with_backoff 2 15 "gNB startup" \
		start_gnb cu_cp_01 1 10.53.1.1 55555 "${exp_dir}/gnb_logs"; then
		error "Failed to start gNB after retries"
		error "Check logs at: ${exp_dir}/gnb_logs/"
		return 1
	fi
	success "gNB cu_cp_01 is running and connected"

	# Wait for network bridge
	info "Waiting for Open5GS bridge interface (10.53.1.1)..."
	...
	while [[ -z "$bridge_name" ]]; do
		...
		if ((elapsed >= bridge_timeout)); then
			...
			error "Current network interfaces:"
			ip -o addr show | sed 's/^/    /'
			return 1
		fi

		bridge_name=$(ip -o addr show | awk '/10\.53\.1\.1/ {print $2; exit}')
		if [[ -n "$bridge_name" ]]; then
			success "Found bridge interface: $bridge_name (took ${elapsed}s)"
			break
		fi
		printf "."
		sleep 2
	...
	# Create namespaces
	info "Creating UE network namespaces..."
	for i in {1..3}; do
		local ue_ns="ue$i"
		if ! ip netns list | grep -q "^${ue_ns}$"; then
			info "Creating namespace: $ue_ns"
	...
	success "All UE namespaces ready"

	# Add routing
	info "Configuring routing to UE network (10.45.0.0/16)..."
	if ! ip route show | grep -q "10.45.0.0/16 via 10.53.1.2"; then
		...
		info "Route to UE network already exists"
	fi
	...
	for i in {1..3}; do
		...
		if ! retry_with_backoff 2 10 "UE$i startup" \
			...
			error "Failed to start UE$i after retries"
			error "Check logs at: ${exp_dir}/ue_logs/ue${i}.log"
			return 1
		fi
		success "UE$i started successfully"
	done
	success "All UEs started"
	...
	if ! bash "$run_scenario_sh"; then
		...
		error "Check scenario script: $run_scenario_sh"
		return 1
	fi
	success "GNU Radio scenario script executed"

	info "Waiting 5s for GNU Radio to initialize..."
	sleep 5

	# Get GNU Radio PID
	info "Checking GNU Radio process..."
	if [[ -f /tmp/python_scenario.pid ]]; then
		...
		if [[ -n "$GNURADIO_PID" ]] && kill -0 "$GNURADIO_PID" 2>/dev/null; then
			...
			error "GNU Radio scenario not running properly (PID: $GNURADIO_PID)"
			error "Process may have died immediately"
			return 1
		fi
	else
		error "GNU Radio PID file not found at /tmp/python_scenario.pid"
		return 1
	fi

	info "Waiting 5s for RF simulation to stabilize..."
	sleep 5

	# Wait for UE connections
	step "Waiting for UE connections"
	for i in {1..3}; do
		info "Waiting for UE$i to connect (timeout: 120s)..."
		if ! wait_for_ue_connection "$i" "${exp_dir}/ue_logs" 120; then
			error "UE$i failed to connect within timeout"
			error "Check UE logs at: ${exp_dir}/ue_logs/ue${i}.log"
			return 1
		fi
		success "UE$i connected successfully"
	done
	success "All UEs connected"

	# Configure UE routes
	info "Waiting 5s before configuring UE routes..."
	sleep 5

	info "Configuring UE routing..."
	for i in {1..3}; do
		local ue_ns="ue$i"
		info "Configuring routing for $ue_ns..."
		if sudo ip netns exec "$ue_ns" ip link show tun_srsue >/dev/null 2>&1; then
			if sudo ip netns exec "$ue_ns" ip route add default via 10.45.1.1 dev tun_srsue 2>/dev/null; then
				success "Configured routing for $ue_ns"
			else
				warn "Failed to configure routing for $ue_ns (may already exist)"
			fi
		else
			warn "Interface tun_srsue not found in $ue_ns"
			error "Available interfaces in $ue_ns:"
			sudo ip netns exec "$ue_ns" ip link show | sed 's/^/    /'
		fi
	done
	success "UE routing configuration complete"
	...
	# Start metrics collection (optional - uncomment to enable)
	# info "Starting KPM metrics collection"
	# docker compose exec python_xapp_runner ./kpm5_xapp.py --ue_ids 0,1,2 > "$metrics_log_file" 2>&1 &
	# local metrics_pid=$!
	...
	# Wait for metrics process to complete (if enabled)
	...
	# Stop the metrics collector (if enabled)
